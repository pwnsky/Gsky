...
void gsky::net::epoll::add(gsky::net::sp_channel sp_channel) {
    ...
    sp_psps_[fd] = sp_channel->get_holder(); // 将sps对象由epoll管理
    event.data.fd = fd;
...
void gsky::net::epoll::mod(sp_channel spc) {
    int fd = spc->get_fd();
...
// 删除channel
void gsky::net::epoll::del(sp_channel sp_channel) {
    int fd = sp_channel->get_fd();
    struct epoll_event event;
    event.data.fd = fd;
...
// 获取所有的Channel
std::vector<gsky::net::sp_channel> gsky::net::epoll::get_all_event_channels() {
...
// 在获取事件后, 获取channel
std::vector<gsky::net::sp_channel> gsky::net::epoll::get_event_channels_after_get_events(int number_of_events) {
